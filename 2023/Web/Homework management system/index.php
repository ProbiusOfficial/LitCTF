    ...
    if ($er) { 
        if (isset($_SESSION['error'])){
            ...
            if($_SESSION['error'] > $safe_num && $safe_num !== 0){
                mail($mail,' 作业管理系统：文件被恶意登录！','该提醒来自 作业管理系统：<br>登录者IP为：'.$_SERVER['REMOTE_ADDR'],'From: <i@hezi.be>');
...
function up() {
    ...
    echo "<div class='title'>远程下载</div><div class='box' style='border-bottom:0;'>什么是远程下载？<br>远程下载是从其他服务器获取文件并直接下载到当前服务器的一种功能。<br>类似于SSH的Wget功能，免去我们下载再手动上传所浪费的时间。<br><form action='{$meurl}?op=yupload' method='POST'>"
         ."</div><input type='text' class='text' style='width:calc(100% - 12px);margin:0;' name='ndir' value='".$_SESSION["folder"]."'><textarea name='url' placeholder='https://github.com/ProbiusOfficial/My_pic/blob/main/demo.jpg'></textarea>"
...
function mainbottom() {
    echo "</div><div style='text-align:center;font-size:13px;color:#999 !important;margin:10px 0 45px 0;font-family:Candara;'>"
        ."Homework management Version 1.10.0.2</div></body>\n"
        ."</html>\n"
        ."<!--默认账户admin admin-->";
    exit;
}
